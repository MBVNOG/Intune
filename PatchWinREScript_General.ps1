...
function IsTPMBasedProtector
{
    ...
    if ($return) {LogMessage("Has TPM-based protector")} else {LogMessage("Doesn't have TPM-based protector")}

    return $return
}

function SetRegistrykeyForSuccess {reg add HKLM\SOFTWARE\Microsoft\PushButtonReset /v WinREPathScriptSucceed /d 1 /f}
...
function TargetfileVersionExam([string]$mountDir)
{
    ...
    switch ($fileVersion) {
        ...
        default {LogMessage("Warning: unsupported OS version")}
    }
    return $hasUpdated
}
...
function PatchPackage([string]$mountDir, [string]$packagePath)
{
    ...
    if ($LASTEXITCODE -eq 0){LogMessage("Successfully applied the package")}
    else
    {
    ...
    if ($LASTEXITCODE -eq 0){LogMessage("Cleanup image succeed")}
    else
    {
...
foreach ($line in $WinREInfo)
{
    ...
    if ($params[1].Lenght -eq 0){continue}
    $content = $params[1].Trim()
    if ($content.Lenght -eq 0){continue}
...
if (Test-Path $mountDir)
{
    ...
    if (!($LASTEXITCODE -eq 0)){LogMessage("Warning: unmount failed: " + $LASTEXITCODE)}
...
if ($LASTEXITCODE -eq 0)
{
    ...
    {
        $hasUpdated = TargetfileVersionExam($mountDir)
        if ($hasUpdated){LogMessage("After patch, find expected version for target file")}
        else{LogMessage("Warning: After applying the patch, unexpected version found for the target file")}
